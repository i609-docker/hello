...
```Dockerfile
FROM docker.io/library/gcc:15.2.0-trixie AS build
COPY ./hello.c /src/
WORKDIR /src/
...
Pour mettre l'image sur un _registry docker_, on peut utiliser la commande [docker push](https://docs.docker.com/reference/cli/docker/image/push/).
...
<ins>Remarque :</ins>  
Pour pouvoir utiliser la commande `docker push`, il faut préalablement s'être authentifié auprès du _registry docker_ en utilisant la commande [docker login](https://docs.docker.com/reference/cli/docker/login/).
...
Dans notre cas, le _pipeline_ comprends 3 étapes.

Une première étape de construction de l'application (l'exécutable _hello_). Elle utilise pour cela, une image docker gcc.
...
On peut faire la même chose sur _GitHub_ en utilisant des [GitHub Actions](https://github.com/features/actions).
